import { PrismaClient, PropertyType, ListingPurpose } from "@prisma/client";
...
async function main() {
  console.log("🔥 STEP 1: Deleting all existing properties, listings, and media...");

  // Delete all existing properties and associated records cleanly
  ...
  console.log("✅ All old listings deleted successfully.");

  console.log("🌐 STEP 2: Seeding geography & admin user...");
  ...
  for (const cData of GLOBAL_COUNTRIES) {
    ...
    for (const cityItem of citiesToProcess) {
      ...
      console.log(`🏠 Seeding 10 Property Types for ${cData.name} (${city.name})...`);
  ...
  console.log("🎉 Complete database re-seed finished successfully!");
}
